...
(function (window, jQuery, undefined) {
    ...
    var StandVerificationTable = {
        options: {
            addButton: '#addverification',
            table: '#datatable-editable-verification',
            problemId: "#proId",
            problemNo: "#problemno",
            approveButton: '#approveVerification',
            rejectButton: '#rejectVerification',
            jobNum: "#vjobnum",
            dialog: {
                wrapper: '#dialog-verification',
                cancelButton: '#dialogCancel-verification',
                confirmButton: '#dialogConfirm-verification',
            }
        },
        userInfo: {
            jobNum: "",
        ...
        initialize: function () {
            StandVerificationTable.setVars()
                .build()
                .events()
                .userAutoComplete();
        },
        ...
        events: function () {
            ...
            this.$table
                .on('click', 'a.save-row', function (e) {
                    ...
                    var params = {
                        Id: data[0].Id || 0,
                        PVProblemId: _self.$problemId.val() || 0,
                        PVProblemNo: _self.$problemNo.html() || '',
                        PVWhat: data[0].What || '',
                        PVWhoNo: data[1].jobnum,
                        PVWho: data[1].name,
                        PVPlanDate: data[2] || '',
                        PVActualDate: data[3] || '',
                        PVWhere: data[4] || '',
                        PVAttachment: data[5].name || 'D',
                        PVAttachmentUrl: data[5].url || 'D',
                        PVStatus: data[6],
                        PVComment: data[7]
                    };
                            ...
                            if (data != null) {
                                if (data.IsSuccess == true) {
                                    var verificationInfo = {
                                        Id: data.data.Id || 0,
                                        What: data.data.PVWhat || '',
                                        JobNo: data.data.PVWhoNo || '',
                                        Name: data.data.PVWho || '',
                                    };
                                    var attachinfo = {
                                        Attachname: (data.data.PVAttachment == "D" ? '' : data.data.PVAttachment) || '',
                                        Attachurl: (data.data.PVAttachmentUrl == "D" ? '' : data.data.PVAttachmentUrl) || '',
                                        Attachdownurl: data.data.PVAttachmentDownloadUrl || '',
                                    };
                                    StandVerificationTable.rowSave($(_event).closest('tr'), verificationInfo, attachinfo);
                                }
                                else {
                                    alert("error");
                                }
            ...
            this.$rejectButton.on('click', function () {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data != null) {
                            if (data.IsSuccess == true) {
                                $("#approveVerificationflag").val(0);
                                SetStandVerificationBtn(1);
                                InitTabs();
                            }
                            else {
                                alert("error");
        ...
        rowSave: function ($row, verificationInfo, attachinfo) {
            var _self = this,
                $actions,
                values = [];
            ...
            values = $row.find('td').map(function () {
                ...
                if ($this.hasClass('actions')) {
                    ...
                    return $.trim("<label class=\"control-label\" data-id=\"" + verificationInfo.Id + "\">" + verificationInfo.What + "</label >");
                }
                else if ($this.hasClass('who')) {
                    return $.trim("<label class=\"control-label\" data-jobno=\"" + verificationInfo.JobNo + "\">" + verificationInfo.Name + "</label >");
                }
                else if ($this.hasClass('status')) {
                        ...
                        xhr.onload = function (result) {
                            ...
                            if (data.data && data.data.length > 0) {
                                ...
                                $(".dropattachment").on('click', function () {
                                    ...
                                    $(drop).parent().remove();
                                    StandVerificationTable.attachmentUpload();
                                    return false;
                                });
                            }
                        };
            ...
            if (userlisturl != '') {
                var options = {
                    ...
                    select: function (event, ui) {
                        $("#vuserSearch").val(ui.item.label);
                        $("#vjobnum").html(ui.item.value);
                        return false;
                    },
                };
                $("#vuserSearch").autocomplete(options);
            }
        },

        setActionBtn: function (flag) {
            if (flag == 1) {
                $("#verificationbody").find("tr").each(function (index, item) {
        ...
        initialdata: function (verificationdata) {
            var datahtml = "";
            StandVerificationTable.datatable.clear();
            $("#verificationbody").html(datahtml);
            for (var i = 0; i < verificationdata.length; i++) {
                var tr = "<tr role=\"row\" class=\"odd\">" +
                    "<td class=\"what\"><label class=\"control-label\" data-id=\"" + verificationdata[i].Id + "\">" + verificationdata[i].PVWhat + "</label></td>" +
                    "<td class=\"who\"><label class=\"control-label\" data-jobno=\"" + verificationdata[i].PVWhoNo + "\">" + verificationdata[i].PVWho + "</label ></td>" +
                    "<td class=\"plandate\">" + verificationdata[i].PVPlanDateDesc + "</td>" +
                    "<td class=\"actualdate\">" + verificationdata[i].PVActualDateDesc + "</td>" +
                    "<td class=\"where\">" + verificationdata[i].PVWhere + "</td>" +
                    "<td class=\"attachment\"><a href=\"" + verificationdata[i].PVAttachmentDownloadUrl + "\" data-value=\"" + verificationdata[i].PVAttachmentUrl + "\" data-download=\"" + verificationdata[i].PVAttachmentDownloadUrl + "\" class=\"fileinfo\">" + verificationdata[i].PVAttachment + "</a ></td>" +
                    "<td class=\"status\">" + verificationdata[i].PVStatus + "</td>" +
                    "<td class=\"comment\">" + verificationdata[i].PVComment + "</td>" +
                ...
                StandVerificationTable.datatable.row.add($(tr));
            }
            StandVerificationTable.datatable.draw();
        }
    };

    window.InitStandVerification = StandVerificationTable.initialize;
    window.InitStandVerificationData = StandVerificationTable.initialdata;
    window.SetStandVerificationBtn = StandVerificationTable.setActionBtn;
})(window, jQuery)
