...
namespace Common
{
    ...
    public class ExcelHelper
    {
        ...
        public void AddRow(Microsoft.Office.Interop.Excel.Worksheet ws, int rowInit)
        {
            ((Range)ws.Rows[rowInit, Type.Missing]).Insert(Type.Missing, XlInsertFormatOrigin.xlFormatFromLeftOrAbove);
        }
        ...
        //设置一个单元格的属性   字体，   大小，颜色   ，对齐方式
        {
            ...
            var r = ws.Range[ws.Cells[Startx, Starty], ws.Cells[Endx, Endy]];
            r.Font.Name = name;
            r.Font.Color = color;
            r.Font.Size = size;
        ...
        //合并单元格
        {
            ...
            Microsoft.Office.Interop.Excel.Range r = ws.Range[ws.Cells[x1, y1], ws.Cells[x1, y1]];
        ...
        public void InsertPictures(string Filename, string ws, int left, int top, float width, float height)
        //插入图片操作一
        {
            GetSheet(ws).Shapes.AddPicture(Filename, MsoTriState.msoFalse, MsoTriState.msoTrue, left, top, width, height);
            //后面的数字表示位置
        }
